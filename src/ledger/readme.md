# What is a ledger?
A ledger represents the state of the epc universe at a given point in time.
...
[`src/xdr/epc-ledger.x`](../xdr/epc-ledger.x)
...
During consensus, nodes work together to decide on the value of epcValue.
epcValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'epcValue' should be able to transition to the same new ledger.
...
[`src/xdr/epc-ledger-entries.x`](../xdr/epc-ledger-entries.x)

### AccountEntry
This entry represents an account. In epc, everything is centered around
...
Offers are a way to automate simple trading inside the epc network.
