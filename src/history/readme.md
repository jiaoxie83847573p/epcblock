...
The `get` and `put` commands define subprocesses that `epc-core` will
...
called `s3epc` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3epc]
get="curl http://history.epc.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.epc.org/{1}"
~~~~

In this example, `epc-core` will use the `curl` command to fetch history and
