...
fuzzer with epc-core. Support for this is still preliminary but it has
...
small inputs in binary form. We've modified epc-core to have a mode for (b)
...
epcMessage into them. This is quite a lot of setup and the fuzzing speed of
...
## Building an instrumented epc-core
...
The simplest way is `make fuzz`; this will do the following:
  ...
  - Run `epc-core --genfuzz fuzz-testcases/fuzz$i.xdr` ten times to produce
    some basic seed input for the corpus.
  - Create a directory `fuzz-findings` for storing crash-producing inputs.
  - Run `afl-fuzz` on `epc-core --fuzz`, using those corpus directories.
...
by feeding it to an instance of `epc-core --fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`epc-core` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`epc-core` in a different directory with `--enable-asan`, or valgrind, in
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `epc-core` itself, not libsodium,
  ...
  - Make startup-modes at different points in the process: instantiate an
    ...
    `epcMessage`s. Let the fuzzer generte bucket ledger entries, and try to
  ...
  - Add a mode -- with a giant red flashing TESTING_ONLY light on it -- that
    ...
    as an attacker who figures out how to break `epc-core` will be able
