...
This document describes changes to the epc protocol as well as other changes made to address security issues.
...
* Changes to SCP.
* Changes to the epc Protocol (anything that modifies how the distributed ledger functions, including historical data).

## Security issues

* DDoS.
* Crashes (that could lead to remote code execution).
* Other attacks that can be exploited (inside or outside of the epc protocol).

## Goals for this document

* have a summary view of changes that affect the code base (epc-core has to be able to replay all ledgers generated since genesis on the epc public network).
...
* this document is not where security disclosures are made, instead follow the process outlined in [epc's bug bounty program](https://www.epc.org/bug-bounty-program/) as a way to triage and respond to issues.
* issues are reflected in this document 30 days after release of the version of epc core containing fixes for the issues.
